		...
		reader.onload = () => {
			...
			if (!config.parsl) {
				...
				if (!executor.provider || executor.provider.class === 'LocalProvider') {
					...
					this.formData.slurm.cpusPerWorker = executor.cores_per_worker
						? executor.cores_per_worker
						: '';
					this.formData.slurm.workersPerNode = executor.max_workers_per_node
						? executor.max_workers_per_node
						: '';
					...
					this.formData.slurm.nodesPerBlock = provider.nodes_per_block
