...
<p>
	...
	scripting library under the hood. Running small workflows in parallel on a laptop requires minimal
	configuration, and can be achieved by simply passing the --p-parallel flag to the pipeline you are
	running without additional configuration; however, QIIME 2 also support parallelizing large scale
	workflows across multiple nodes on a compute cluster. This is particularly useful for pipelines in
	MOSHPIT and q2-boots which often do the same type of calculation a very large number of times on a
	large set of data.
</p>
<br />
<p>
	This webapp can help you create a file to pass into the --p-parallel-config parameter of a QIIME 2
	Pipeline you want to run in parallel on an HPC cluster using the slurm scheduler. PARSL and thus
	QIIME 2 do support schedulers other than slurm, but this app currently only supports creating
	slurm configs. More information about QIIME 2 parallel configuration may be found <a
...
<form onsubmit={writeConfig}>
	{#key $formModel.configType}
		...
		{#if formModel.configType == 'local'}
			<label for="localExecutorType"
				>Is the action you are running threadsafe? (If you don't know choose no.)</label
			>
			<select
			...
			<br />
			<label for="numTasks"
				>How many threads/processes would you like to run Note: More threads/processes means more
				memory usage</label
			>
			<input
				id="numTasks"
		...
		{:else}
			...
			<br />
			<label for="walltime"
				>What would you like the walltime for your Slurm jobs (maximum runtime of each job) to be in
				HH:MM:SS?</label
			>
			<input
				id="walltime"
			...
			/>
			<br />
			<label for="workerInit"
				>What command(s) do you need to run to activate QIIME 2? If there are multiple commands
				please seperate them with ;</label
			>
			<input
				id="workerInit"
